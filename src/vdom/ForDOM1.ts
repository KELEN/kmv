...
export class ForDOM {
    ...
    reRenderList (kmv) {
        ...
        while (startElem !== endElem.previousSibling) {
            ...
            let childrenVdom = startElem.childrenVdom;
            childrenVdom.$dom.innerHTML = Math.random();
            for (let i = 0; i < childrenVdom.length; i++) {
                let cvdom = childrenVdom[i];
                switch (cvdom.nodeType) {
                    case NodeType.TEXT:
                        DomUtil.changeTextContent(cvdom.$dom, compileTpl(cvdom.template, data));
                        break;
                }
            }
            index ++;
